...
for k=1:3
   ...
   y_lognorm(k,:)= PL_logdist_or_norm(fc,distance,d0,Exp(1),sigma); % shadowing
end
subplot(131)
...
xlabel('Distance[m]'), ylabel('Path loss[dB]'), legend('n=2','n=3','n=6')
subplot(133)
...
xlabel('Distance[m]'), ylabel('Path loss[dB]'), legend('path 1','path 2','path 3')